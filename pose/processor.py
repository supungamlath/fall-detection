...
class Processor(object):
    ...
    def single_image(self, image):
        ...
        processed_image = openpifpaf.datasets.PilImageList([im], preprocess=preprocess)[
            0
        ][0]
        ...
        all_fields = self.processor.batch(
            self.model, torch.unsqueeze(processed_image.float(), 0), device=self.device
        )[0]
